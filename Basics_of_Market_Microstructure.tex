...
\[
    \text{Total Cost} = \underbrace{\sum x_j d_j (p_j - m_d)}_{\text{Execution Cost}} + \underbrace{(X - \sum x_j) d (p_N - m_d)}_{\text{Opportunity Cost}}
\]
Where:
\begin{itemize}
    \item $X$: Total order size (capital).
    \item $\sum x_j$: Total number of shares actually bought or sold.
    \item $p_N$: Last price of the day.
    \item $m_d$: Decision-time bid-ask midpoint.
\end{itemize}
The difference between $X$ and $\sum x_j$ represents the unexecuted portion of the order. The non-execution results in an opportunity cost due to potentially lost profits (captured by $p_N - m_d$). For a buy order, it assumes you could have bought at $m_d$ and sold at $p_N$. If prices increase, this is lost profit; if prices decrease, it is losses avoided.

...
\subsection{Introduction}
Even in the most liquid markets, investors bear transaction costs which reduce net returns. Understanding and measuring these costs is critical for efficient trading.
\begin{itemize}
    \item \textbf{Investors:} Use transaction cost analysis to evaluate the execution quality of their brokers and strategies.
    \item \textbf{Exchanges:} Calculate costs to measure liquidity and market their platform's efficiency.
    \item \textbf{Brokers:} Use analysis to optimize execution strategies and minimize costs for clients.
    \item \textbf{Regulators:} Monitor exchange performance and implement policies to reduce market friction.
\end{itemize}

\subsection{Measuring Transaction Costs}
Transaction costs are measured as the difference between the theoretical value of a trade and the actual realized value.
\begin{itemize}
    \item \textbf{Paper Portfolio:} Theoretical value assuming trades execute at the decision-time midpoint (frictionless).
    \item \textbf{Real Portfolio:} Actual value accounting for frictions (spreads, commissions, price impact, taxes).
    \item \textbf{Transaction Cost} = Value of Paper Portfolio $-$ Value of Real Portfolio.
\end{itemize}
...
Actual out-of-pocket expenses incurred during trading.
\begin{itemize}
    \item \textbf{Examples:} Brokerage commissions, taxes, and exchange fees.
    \item \textbf{Measurement:} Easy to identify and sum up.
\end{itemize}

\subsubsection{Implicit Costs}
Non-obvious costs imposed on traders, often related to market mechanics.
\begin{itemize}
    \item \textbf{Examples:} Bid-ask spread, market impact, opportunity cost, and delay cost.
    \item \textbf{Measurement:} Difficult to quantify because they require estimating a counterfactual (e.g., what the price would have been without the trade).
\end{itemize}

\subsection{Calculating Implicit Costs}
Implicit costs are calculated by comparing the trade price to a specific benchmark.
...
\begin{itemize}
    \item \textbf{Interpretation:} Measures the premium paid (buying) or discount accepted (selling) relative to the benchmark ("true value").
    \item \textbf{Round-Trip Cost:} $2 \times$ One-Way Implicit Cost.
\end{itemize}
...
Uses the midpoint of the bid and ask prices at the time the investor \textbf{decides} to trade ($m_d$). This method captures both execution costs and opportunity costs.

\textbf{1. Execution Cost (Executed Portion):}
\[
    \text{Execution Cost} = \sum x_j d_j (p_j - m_d)
\]

\textbf{2. Opportunity Cost (Unexecuted Portion):}
\[
...
\textbf{Total Implementation Shortfall} = Execution Cost + Opportunity Cost.
\begin{itemize}
    \item Captures the cost of failing to execute the full order size.
    \item If prices move favorably (e.g., rise for a buy order), the unexecuted portion represents a lost profit opportunity.
\end{itemize}

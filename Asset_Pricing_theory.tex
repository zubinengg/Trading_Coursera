...
If the holding period is not exactly one year, we often annualize the return to make it comparable.
\[
    R_{annual} = (1 + R_{period})^{1/n} - 1
\]
Where $n$ is the number of years. For example, if you earned 12\% over 6 months ($n=0.5$), the annualized return is $(1.12)^{1/0.5} - 1 \approx 25.4\%$.
...
\textbf{Role of Correlation:}
Diversification works best when assets are \textbf{uncorrelated} (correlation $\rho < 1$).
\begin{itemize}
    \item If two stocks move perfectly together ($\rho = +1$), combining them does not reduce risk.
    \item If they move independently ($\rho = 0$) or oppositely ($\rho = -1$), risk is significantly reduced.
\end{itemize}
...
\subsubsection{Real-World Example: Apple Inc.}
Suppose we want to calculate the expected return for Apple (AAPL) using CAPM.
\begin{itemize}
    \item \textbf{Risk-Free Rate ($r_f$):} 2.0\% (Yield on 10-year US Treasury).
    \item \textbf{Market Return ($E[r_m]$):} 8.0\% (Historical average of S\&P 500).
    \item \textbf{Beta ($\beta$):} 1.2 (Apple is slightly more volatile than the market).
\end{itemize}
\[
    E[R_{AAPL}] = 2.0\% + 1.2 (8.0\% - 2.0\%) = 2.0\% + 7.2\% = 9.2\%
\]
...
\textbf{Industry Sensitivity:}
Different industries have different sensitivities ($\beta$) to factors.
\begin{itemize}
    \item \textbf{Airline Company:} High sensitivity to \textbf{Oil Prices} (Factor 1) and \textbf{GDP Growth} (Factor 2).
    \item \textbf{Tech Company:} Low sensitivity to Oil Prices, but high sensitivity to \textbf{Interest Rates} (Factor 3).
\end{itemize}
...
\section{Calculating Arbitrage-Free Prices and APT Drawbacks}

\subsection{Learning Outcomes}
After this section, you will be able to:
\begin{itemize}
    \item Solve for risk premiums given factor sensitivities.
    \item Determine the arbitrage-free price of an asset given the prices of other risky assets.
    \item Discuss a key drawback of the APT regarding relative pricing.
\end{itemize}

\subsection{Step 2: Solving for Risk Premiums}
Continuing from the previous example, we have the expected returns and betas for Microsoft and Intel (assuming they are correctly priced):
\begin{itemize}
    \item \textbf{Microsoft:} $E[r_M] = 20\% = 0.20$, $\beta_M = 0.8$.
    \item \textbf{Intel:} $E[r_I] = 50\% = 0.50$, $\beta_I = 3$.
\end{itemize}

Using the APT equation $E[r] = r_f + \beta (RP)$, we set up a system of two equations with two unknowns ($r_f$ and $RP$):
\begin{align*}
    0.20 & = r_f + 0.8(RP) \\
    0.50 & = r_f + 3(RP)
\end{align*}
...
\[
    0.30 = 2.2(RP) \implies RP = \frac{0.30}{2.2} \approx 0.1364 \text{ or } 13.64\%
\]
Substituting $RP$ back into the first equation:
\[
    r_f = 0.20 - 0.8(0.1364) = 0.20 - 0.1091 = 0.0909 \text{ or } 9.09\%
\]
So, the implied risk-free rate is 9.09\% and the factor risk premium is 13.64\%.

\subsection{Step 3: Determining the Arbitrage-Free Price of Coke}
Now we determine the correct price ($P$) for Coke using the APT parameters derived above.
\begin{itemize}
    \item \textbf{Coke's Payoffs:} Good State = 55, Bad State = 15.
    \item \textbf{APT Requirement:} $E[r_C] = 0.0909 + 0.1364 \beta_C$.
\end{itemize}

We express Coke's expected return and beta in terms of its unknown price $P$.
\begin{itemize}
    \item Return (Good): $\frac{55}{P} - 1$.
    \item Return (Bad): $\frac{15}{P} - 1$.
\end{itemize}
Using the single factor model decomposition (shocks: Good $+0.5$, Bad $-0.5$):
\begin{align*}
    \frac{55}{P} - 1 & = E[r_C] + 0.5 \beta_C \\
    \frac{15}{P} - 1 & = E[r_C] - 0.5 \beta_C
\end{align*}
Solving for $E[r_C]$ and $\beta_C$:
\begin{itemize}
    \item Summing: $\frac{70}{P} - 2 = 2 E[r_C] \implies E[r_C] = \frac{35}{P} - 1$.
    \item Subtracting: $\frac{40}{P} = \beta_C$.
\end{itemize}

Substitute these into the APT equation:
\[
    \left( \frac{35}{P} - 1 \right) = 0.0909 + 0.1364 \left( \frac{40}{P} \right)
\]
\[
    \frac{35}{P} - 1 = 0.0909 + \frac{5.456}{P}
\]
\[
    \frac{35 - 5.456}{P} = 1.0909
\]
\[
    P = \frac{29.544}{1.0909} \approx 27.08
\]
The arbitrage-free price of Coke is \textbf{27.08}.

\subsection{Verifying the Arbitrage Portfolio}
If Coke is priced at 27.08, let's check the portfolio constructed earlier (Buy 1 Coke, Short 0.5 Microsoft, Short 0.5 Intel).
\begin{itemize}
    \item \textbf{Cost:} $27.08 - 0.5(25) - 0.5(20) = 27.08 - 12.5 - 10 = 4.58$.
    \item \textbf{Payoff:} Always 5 (Risk-free).
    \item \textbf{Return:} $\frac{5}{4.58} - 1 \approx 0.0917 \approx 9.1\%$.
\end{itemize}
This matches the implied risk-free rate ($9.09\%$), confirming no arbitrage exists at this price.

\subsection{Drawbacks of APT: Relative Pricing}
APT relies on the assumption that the benchmark assets are correctly priced. If we choose different pairs of assets as our "correct" benchmarks, we get different results.

\begin{itemize}
    \item \textbf{Assuming Intel \& Coke are correct:} $r_f = 63.64\%$, $RP = -4.55\%$.
    \item \textbf{Assuming Microsoft \& Coke are correct:} $r_f = -9.09\%$, $RP = 36.36\%$.
\end{itemize}

\textbf{Conclusion:}
\begin{itemize}
    \item APT is about relative pricing. It ensures internal consistency but does not guarantee that the entire market is priced correctly in absolute terms.
    \item Depending on the reference assets, implied parameters ($r_f, RP$) can be unrealistic (e.g., negative risk-free rates).
\end{itemize}

\noindent\rule{\linewidth}{1pt}
%-----------------------------------------------------------------

\section{Commonly Used Risk Factors}

\subsection{Learning Outcomes}
After this section, you will be able to:
\begin{itemize}
    \item Describe commonly used risk factors.
    \item Briefly explain why they work.
\end{itemize}

\subsection{The Fama-French Three Factor Model (1993)}
While APT allows for multiple factors, it does not specify what they are. Eugene Fama and Ken French proposed three specific risk factors that have become the standard in empirical finance.

\subsubsection{The Three Factors}
\begin{enumerate}
    \item \textbf{RMRF (Market Risk Premium):} The return on a market index minus the risk-free rate ($R_m - R_f$). This captures the systematic risk of the market originating from macroeconomic factors.
    \item \textbf{SMB (Small Minus Big):} The return on a portfolio of small companies (low market capitalization) minus the return on a portfolio of big companies (high market capitalization).
    \item \textbf{HML (High Minus Low):} The return on a portfolio of firms with high book-to-market ratios minus a portfolio of firms with low book-to-market ratios.
\end{enumerate}

\subsubsection{Economic Rationale}
\begin{itemize}
    \item \textbf{SMB:} Small firms are likely to be more sensitive to sudden changes in business conditions than big firms.
    \item \textbf{HML:} Firms with high book-to-market ratios are often in financial distress or are "value" stocks, while low ratios indicate "growth" stocks with high future opportunities.
\end{itemize}

\subsubsection{Understanding Book-to-Market Ratio}
\[
    \text{Book-to-Market} = \frac{\text{Book Value of Equity}}{\text{Market Value of Equity}}
\]
\begin{itemize}
    \item \textbf{Low Ratio (Growth):} Reflects high growth opportunities. The market capitalizes these future opportunities, leading to a high market value relative to book value.
    \item \textbf{High Ratio (Value/Distress):} Reflects potential bad financial health or undervaluation. The market value is low relative to book value.
\end{itemize}

\subsection{Empirical Nature of the Model}
Unlike CAPM or APT which are theoretical, the Fama-French model is \textbf{empirical}.
\begin{itemize}
    \item \textbf{Justification:} SMB and HML are not obvious risk factors, but they capture sensitivity to unknown macroeconomic risks.
    \item \textbf{Performance:} The model has worked well for the last 90 years across global markets.
    \item \textbf{Limitation:} There is no guarantee it will continue to work in the future.
\end{itemize}

\subsection{Carhart Four Factor Model (1997)}
Mark Carhart extended the model by adding a fourth factor based on price persistence.
\begin{itemize}
    \item \textbf{MOM (Momentum):} The return on a portfolio of recent "winners" (companies that performed well in the recent past) minus a portfolio of recent "losers".
    \item \textbf{Observation:} Recent winners tend to continue performing well, while losers continue to perform poorly.
\end{itemize}

\subsection{Other Asset Pricing Models}
Beyond the standard CAPM, APT, and Fama-French models, researchers have developed other frameworks to explain asset returns.

\subsubsection{Consumption-Based CAPM (CCAPM)}
\begin{itemize}
    \item \textbf{Concept:} Assets are priced based on their covariance with aggregate consumption growth rather than the market portfolio.
    \item \textbf{Vs. CAPM/APT:} Focuses on the utility of consumption over time rather than just wealth. It provides a stronger theoretical economic foundation but performs poorly empirically.
\end{itemize}

\subsubsection{Intertemporal CAPM (ICAPM)}
\begin{itemize}
    \item \textbf{Concept:} Developed by Robert Merton, it assumes investors hedge against future changes in investment opportunities (e.g., changes in interest rates or volatility).
    \item \textbf{Vs. CAPM/APT:} CAPM is a single-period model. ICAPM is multi-period and adds "hedging factors" to the market factor.
\end{itemize}

\subsubsection{Liquidity-Adjusted CAPM}
\begin{itemize}
    \item \textbf{Concept:} Incorporates the risk that an asset cannot be traded quickly without a significant price impact.
    \item \textbf{Vs. CAPM/APT:} Standard CAPM and APT assume frictionless markets (no transaction costs). Liquidity models explicitly incorporate market frictions.
\end{itemize}

\subsubsection{Behavioral Asset Pricing}
\begin{itemize}
    \item \textbf{Concept:} Incorporates psychological biases (e.g., overconfidence, loss aversion) and limits to arbitrage.
    \item \textbf{Vs. CAPM/APT:} Standard models assume rational investors and efficient markets. Behavioral models relax these assumptions to explain anomalies.
\end{itemize}

\subsection{Summary}
Together, these four factors (RMRF, SMB, HML, MOM) historically explain over \textbf{80\%} of the variation in stock returns.


\noindent\rule{\linewidth}{1pt}
%-----------------------------------------------------------------
\section{Test Solution}
\section*{Question 1: Expected Return of GEM Inc}

The one-year return distribution of GEM Inc is given as follows:
...
\[
    E(R) = (-15)(0.10) + (-5)(0.20) + (0)(0.25) + (5)(0.15) + (15)(0.30)
\]

\[
    E(R) = -1.5 - 1 + 0 + 0.75 + 4.5 = 2.75\%
\]

\textbf{Expected Return = 2.75\%}

\section*{Question 2: Volatility (Standard Deviation)}

Using the expected return from Question 1:
\[
    E(R) = 2.75\%
\]
...
\begin{table}[H]
    ...
    \begin{tabular}{ccccc}
        \toprule
        Return & Probability & $r_i - E(R)$ & $(r_i - E(R))^2$ & Weighted \\
        \midrule
        -15    & 0.10        & -17.75       & 315.06           & 31.51    \\
        -5     & 0.20        & -7.75        & 60.06            & 12.01    \\
        0      & 0.25        & -2.75        & 7.56             & 1.89     \\
        5      & 0.15        & 2.25         & 5.06             & 0.76     \\
        15     & 0.30        & 12.25        & 150.06           & 45.02    \\
        \bottomrule
    \end{tabular}
\end{table}
...
\[
    \sigma = \sqrt{91.19} \approx 9.55\%
\]

\textbf{Volatility = 9.549\%}

\section*{Question 3: Systematic Risk of Stock 1}

Given:
\[
    \beta_1 = 1.2, \quad \sigma_m^2 = 0.09
\]

Systematic risk under the single-factor model is:
\[
    \text{Systematic Risk} = \beta^2 \sigma_m^2
\]

\[
    = (1.2)^2 \times 0.09 = 0.1296
\]

\textbf{Systematic Risk = 0.1296}

\section*{Question 4: Expected Return of Stock Y (No-Arbitrage)}
...
Assuming a linear pricing model:
\[
    E(R) = a + b\beta
\]
...
\textbf{Mean Return of Stock Y = 35\%}

\section*{Question 5: Arbitrage Portfolio}

An arbitrage portfolio under APT:
\begin{itemize}
    \item Requires zero initial investment
    \item Has no risk
    \item Generates non-negative expected return
\end{itemize}

\textbf{Correct Answer: No risk}

\section*{Question 6: Pure Factor Portfolio}

A pure factor portfolio has:
\begin{itemize}
    \item Unit sensitivity to its factor
    \item Zero sensitivity to all other factors
\end{itemize}

\textbf{Correct Answer: Unit sensitivity}

\section*{Question 7: Risk-Free Rate (Two-Factor APT)}

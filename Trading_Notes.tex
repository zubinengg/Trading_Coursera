...
This section covers the common expenses a company incurs while generating revenues, such as Cost of Goods Sold (COGS) and Operating Expenses. It also introduces intermediate profit measures.

\subsection{Cost of Goods Sold (COGS)}
COGS refers to the direct costs a company incurs to generate products ready strictly for sale.
\begin{itemize}
    \item \textbf{Includes:} Raw material costs and direct labor costs involved in production.
    \item \textbf{Excludes:} Distribution costs and sales force costs.
    \item \textbf{Amazon Case Study (2015):} Reported as "Cost of Sales" (\$71.65 billion).
          \begin{itemize}
              \item Includes purchase price of consumer products, digital media content, packaging, and shipping costs (inbound/outbound).
              \item Essentially all costs incurred to deliver products to customers.
          \end{itemize}
\end{itemize}

\subsection{Gross Profit}
Gross Profit is the difference between specific Revenues and COGS.
\begin{equation*}
    \text{Gross Profit} = \text{Net Sales} - \text{COGS}
\end{equation*}
\begin{itemize}
    \item Represents profit after deducting costs directly associated with producing/selling the product or service.
    \item \textbf{Amazon (2015):} \$107.01 B (Sales) - \$71.65 B (COGS) = \$35.36 B.
\end{itemize}

\subsection{Operating Expenses}
Operating expenses are deducted from Gross Profit to determine Operating Income. The major component is often SG\&A.

\subsubsection{Selling, General, and Administrative (SG\&A)}
Non-production costs related to selling products and general operations, including executive salaries and general support.
\begin{itemize}
    \item \textbf{Amazon (2015):} Total SG\&A of \$20.41 billion.
    \begin{itemize}
        \item \textit{Fulfillment (\$13.41 B):} Primarily includes costs incurred in operating and staffing fulfillment and customer service centers, as well as payment processing centers.
        \item \textit{Marketing (\$5.25 B):} Costs incurred in marketing and selling products and services.
        \item \textit{General \& Administrative (\$1.75 B):} Includes payroll expenses, professional and litigation expenses, other corporate costs, as well as depreciation expenses.
    \end{itemize}
\end{itemize}

\subsubsection{Depreciation and Amortization (D\&A)}
Expenses related to the usage of capitalized assets over their useful life.
\begin{itemize}
    \item \textbf{Depreciation:} Related to PP\&E (tangible assets).
    \item \textbf{Amortization:} Related to intangible assets (goodwill, trademarks).
    \item \textbf{Non-Cash Expense:} No cash is paid out when this expense is recorded.
    \item \textbf{Reporting:} Often embedded within SG\&A components (as done by Amazon). Amazon's Cash Flow statement reported \$6.28 B in D\&A for 2015.
\end{itemize}

\subsubsection{Research and Development (R\&D)}
Costs incurred in developing new products or services.
\begin{itemize}
    \item \textbf{Amazon (2015):} Reported as "Technology and Content" (\$12.54 billion). Includes payroll for application/platform development and technology infrastructure.
\end{itemize}

\subsection{Operating Profit (EBIT)}
Operating Profit, or Earnings Before Interest and Taxes (EBIT), represents the company's profit from core operations.
...
\begin{itemize}
    \item \textbf{Total Operating Expenses:} \$20.41 B (SG\&A) + \$12.54 B (Tech) + \$0.17 B (Other) = \$33.12 B.
    \item \textbf{Operating Profit:} \$35.36 B (Gross Profit) - \$33.12 B (Op Expenses) = \$2.24 B.
\end{itemize}
...
\end{equation*}
\begin{itemize}
    \item A more relevant measure of core profitability as it adds back non-cash expenses.
    \item \textbf{Amazon (2015):} \$2.24 B (EBIT) + \$6.28 B (D\&A) = \$8.52 B.
\end{itemize}
